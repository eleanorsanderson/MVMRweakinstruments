...
pi2 = as.matrix(c(runif((L/2),0,pimax), runif((L/2),0,0)),nrow=L,ncol=n)
pi1 = as.matrix(c(runif((L/2),0,0), runif((L/2),0,pimax)),nrow=L,ncol=n)
alpha = as.matrix(c(rnorm(L/2,0,tau_true), rep(0,L/2)),nrow=L,ncol=1)  


source("simulations_2ex_nc.R")
