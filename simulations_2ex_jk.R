...
#  QIVW2_LIML[j] <- optimize(Q_ind2, interval = c(-100,100))$objective
 ...
 ################################################
  ...
  liml1_jk = liml2_jk = Qexact_jk = limlh1_jk = limlh2_jk = Qexact_het_jk = NULL
  ...
  for(t in 1:jkreps){
    ...
    stderr <- stderr2
    stderr[t,] <- NA
    stderr <- na.omit(stderr)
    
    gammahat <- gammahat2
    gammahat[t] <- NA
    gammahat <- na.omit(gammahat)
    
    segamma <- segamma2
    segamma[t] <- NA
    segamma <- na.omit(segamma)
    
  #LIML estimation of the betas.
  
  liml_beta<- optim(c(0,0), LIML_b)
  
  liml1_jk[t] <- liml_beta$par[1]
  liml2_jk[t] <- liml_beta$par[2]
  
  Qexact_jk[t] <- liml_beta$value
  ...
  liml_het2<- optim(c(0,0), PL2_MVMR)
  limlh1_jk[t] <- liml_het2$par[1]
  limlh2_jk[t] <- liml_het2$par[2]
  Qexact_het_jk[t] <- liml_het2$value
  

  }
  jktemp <- data.frame( liml1_jk, liml2_jk, Qexact_jk, limlh1_jk, limlh2_jk, Qexact_het_jk)
  
  jk_mean <- apply(jktemp, 2, mean)
  jk_sd <- apply(jktemp, 2, sd)
  
  jackknife <- rbind(jackknife, c(jk_mean, jk_sd))
